...
map("n", "<C-i>", "<C-i>", { noremap = true, desc = "Go back in the jumplist" })
map("n", "<C-o>", "<C-o>", { noremap = true, desc = "Go forth in the jumplist" })
...
map("i", "<M-h>", "<Left>", opts("I Move to the left"))
map("i", "<M-j>", "<C-o>j", opts("Move down"))
map("i", "<M-k>", "<C-o>k", opts("Move up"))
-- warcrime; I do this because "<C-o>l" just ends up in the same place before an end of line! think about it
map("i", "<M-l>", "<Right>", opts("Move right"))
map("i", "<M-a>", "<C-o>A", opts("Insert at the end"))
map("i", "<M-i>", "<C-o>I", opts("Insert at the end"))
map("i", "<M-e>", "<C-o>e<Right>", opts("End of word"))
map("i", "<M-b>", "<C-o>b", { noremap = false, desc = "Beginning of word" })
map("i", "<M-w>", "<C-o>w", opts("Next word"))
map("i", "<M-x>", "<C-o>dl", {desc="Delete next character", noremap=false})
map("i", "<M-d>", "<C-o>d", {desc="Delete (expects motion)", noremap=false})
map("i", "<M-S-b>", "<C-o><S-b>", opts("Beginning of WORD"))
map("i", "<M-S-e>", "<C-o><S-e><Right>", opts("End of WORD"))
map("i", "<M-S-w>", "<C-o><S-w>", opts("Next WORD"))
...
map("n", "<C-q><C-e>", ":q!<CR>", options)
map("n", "<C-q><C-q>", ":copen<CR>", options)
...
map({ "n", "v" }, "<M-s>", "<C-w>v", options)
map({ "n", "v" }, "<M-v>", "<C-w>s", options)
...
map("v", "v", function()
	require("utils.motions").feedkeys_and_return_cursor({ "o", "<esc>", "V" })
end, opts("Switch search to visual-line"))
map("v", ",", "mQ:norm A,<CR>`Q", opts("Append commas to the end of all selected lines."))

-- Neorg
map("n", "<leader>nJ", ":Neorg journal today<CR>", opts("Journal"))
map("n", "<leader>nn", ":Neorg index<CR>", opts("Index"))
